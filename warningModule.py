...
Parameters ={
            "Gas":
                {"MP":{"low":np.array([80, 100, 20]),
                        ...
                        "warning":'Warning: There is a Medium Pressure Gas Line in this Area| ',
                        "z_thresh": 0}, 
                "LHP":{"low":np.array([10, 150, 20]),
                        "high":np.array([20, 255, 255]),
                        "warning":'Warning: There is an LHP Pressure Gas Line in this Area | ',
                        "z_thresh": 20},
                "IP":{"low":np.array([50, 100, 20]),
                        "high":np.array([65, 255, 255]),
                        "warning":'Warning: There is an Intermediate Pressure Gas Line in this Area | ',
                        "z_thresh": 20}
                },
            "Electricity":
                {"11KV":{"low":np.array([0, 50, 50]),
                        "high":np.array([10, 255, 255]),
                        "warning": 'Warning: There is a 11kV High Voltage Electricity Line in this Area | ',
                        "z_thresh": 0}, 
                "33KV":{"low":np.array([50, 100, 20]),
                        "high":np.array([65, 255, 255]),
                        "warning":'Warning: There is a 33kV High Voltage Electricity Line in this Area | ',
                        "z_thresh": 0},
                "66KV":{"low":np.array([10, 100, 20]),
                        "high":np.array([20, 200, 255]),
                        "warning":'Warning: There is a 66kV High Voltage Electricity Line in this Area| ',
                        "z_thresh": 20},
                "132KV":{"low":np.array([140, 150, 20]),
                        "high":np.array([160, 255, 255]),
                        "warning":'Warning: There is a 132kV High Voltage Electricity Line in this Area| ',
                        "z_thresh": 20},
                },
            "Water":{
                "Mains":{
                ...
                "Contaminated":{
                    ...
                    "warning":'Warning: There is a Wastewater Sewer in this Area|',
                    "z_thresh":0
                    }
                }
            }
...
def water_map_warning(image,template, method):
    '''
    Input parameters:
...
def inputfunction(filename_pdf, utility_type, packFolderPath):
    ...
    warning_str_total = ''
    warning_dict = {'Gas':"", 'Electricity':"", 'Water':""}
    ...
    if utility_type in warning_dict.keys():
        ...
        for i in range(number_of_files):
            ...
            if utility_type=='Water':
                warning_str_total+= water_map_warning( image = cv2.cvtColor(ori_img, cv2.COLOR_BGR2GRAY),template='protected_region.jpg' , method = 'cv2.TM_CCOEFF_NORMED')


            if (warning_str_total == ''):
                warning_str_total = " No Warning detected by AI Module for "+str(utility_type)
